...
{
    glUseProgram(d.shader_program);
    if (d.update_fn_vec)
        d.update_fn_vec(d.uniform_location, 1, d.data);
    else
        d.update_fn_mat(d.uniform_location, 1, false, d.data);
}
                               ...
                               void *update_fn_vec, void *update_fn_mat)
{
    struct datawloc d = {0};
    d.data = p;
    d.shader_program = shader_program;
    d.uniform_location = glGetUniformLocation(shader_program, name);
    d.update_fn_vec = update_fn_vec;
    d.update_fn_mat = update_fn_mat;
    updateUniform(d);
    return d;
}